...
'	display: block; ' +
'}' +
'.th-media-ads .th-media-ads-skip-block {' +
'	display: block; ' +
'	position: absolute;' +
'	right: 0;' +
'	top: 0;' +
...
'	color: #fff; ' +
'}' +
'.th-media-ads .th-media-ads-skip-button {' +
'	cursor: pointer; ' +
'}' +
'.th-media-ads .th-media-ads-skip-button:hover {' +
...
'}' +
	...
	$.extend(MediaElementPlayer.prototype, {
		...
		buildads: function(player, controls, layers, media) {
			...
			player.adsLayer =
					$('<div class="th-media-layer th-media-overlay th-media-ads">' +
							'<a href="#" target="_blank">&nbsp;</a>' +
							'<div class="th-media-ads-skip-block">' +
								'<span class="th-media-ads-skip-message"></span>' +
								'<span class="th-media-ads-skip-button">' + mejs.i18n.t('th-media.ad-skip') + '&raquo;</span>' +
							'</div>' +
						'</div>')
							.insertBefore( layers.find('.th-media-overlay-play') )
							.hide();

			player.adsLayer.find('a')
						.on('click', $.proxy(t.adsAdClick, t) );

			player.adsSkipBlock = player.adsLayer.find('.th-media-ads-skip-block').hide();
			player.adsSkipMessage = player.adsLayer.find('.th-media-ads-skip-message').hide();

			player.adsSkipButton = player.adsLayer.find('.th-media-ads-skip-button')
		...
		adsPrerollMeta: function() {
			...
			setTimeout(function() {
				t.controls.find('.th-media-duration').html(
		...
		adsPrerollStarted: function() {
			...
			if (t.options.adsPrerollAdEnableSkip) {
				...
				if (t.options.adsPrerollAdSkipSeconds > 0) {
					...
					t.adsSkipMessage.html( mejs.i18n.t('th-media.ad-skip-info').replace('%1', t.options.adsPrerollAdSkipSeconds.toString()) ).show();
		...
		adsPrerollUpdate: function() {
			...
			if (t.options.adsPrerollAdEnableSkip && t.options.adsPrerollAdSkipSeconds > 0) {
				...
				if (t.media.currentTime > t.options.adsPrerollAdSkipSeconds) {
					...
					t.adsSkipMessage.html(mejs.i18n.t('th-media.ad-skip-info').replace('%1', Math.round( t.options.adsPrerollAdSkipSeconds - t.media.currentTime ).toString()) )
				}
