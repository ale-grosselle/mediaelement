(function($) {
	...
	$.extend(MediaElementPlayer.prototype, {
		buildbacklight : function(player, controls, layers, media) {
			...
			var
				mediaContainer = player.container.find('.th-media-mediaelement').parent(),
				border = $('<div class="th-media-border"></div>')
				...
				glowBase = $('<div class="th-media-backlight-glow"></div>')
					.prependTo(mediaContainer)
					.css('position','absolute')
					.css('display','none')
					.css('top',0)
					.css('left',0)
					.width(player.width).height(player.height),
				base = $('<div class="th-media-backlight"></div>')
					.prependTo(mediaContainer)
					.css('position','absolute')
					.css('top',0)
					.css('left',0)
			...
			// add toggle control
			$('<div class="th-media-backlight-button th-media-backlight-active"><span></span></div>')
				.appendTo(controls)
				.click(function() {
					if (isActive) {
						delete timer;
						...
						glowBase.hide();
						$(this)
							.removeClass('th-media-backlight-active')
							.addClass('th-media-backlight-inactive')
					} else {
						updateLights();
						base.show();
						glowBase.show();
						$(this)
							.removeClass('th-media-backlight-inactive')
							.addClass('th-media-backlight-active')
					}
					isActive = !isActive;
				});
